...
MainWindow::MainWindow()
{
    ...
    reserve_struct.columns = form_columns_struct("MANAGER_RESERVE");
...
void MainWindow::closeEvent(QCloseEvent *e){
    if(QMessageBox::question(this, "Выйти?", "Выйти из программы?", "Да", "Нет") == 0){
        ...
        save_columns_struct(reserve_struct.columns, "MANAGER_RESERVE");
        settings->sync();
        delete mwidget;
        delete settings;
...
void MainWindow::create_menu(){
    ...
    menu = menuBar()->addMenu("Списки");
    if(prices_access){
        menu->addAction(contragents_list_action);
    }
    ...
    menu = menuBar()->addMenu("Журнал документов");
    menu->addAction(reserves_list_action);
...
void MainWindow::create_actions(){
    ...
    contragents_list_action = new QAction("Контрагенты", this);
    contragents_list_action->setStatusTip("Открыть список контрагентов");
    connect(contragents_list_action, SIGNAL(triggered()), this, SLOT(open_contragents_list()));

    reserves_list_action = new QAction("Резерв", this);
    reserves_list_action->setStatusTip("Список документов, содержащих деталь в резерве");
    connect(reserves_list_action, SIGNAL(triggered()), this, SLOT(open_reserves_list()));
}
...
void MainWindow::open_contragents_list(){
    ContragentsList *contragents = new ContragentsList(false, 0);
    mwidget->addWidget(contragents);
    mwidget->setCurrentWidget(contragents);
    contragents->show();
    doubtful_widget = contragents;
    connect(contragents, SIGNAL(delete_me()), SLOT(close_contragents_list()));
}

void MainWindow::close_contragents_list(){
    mwidget->removeWidget(mwidget->currentWidget());
    //delete doubtful_widget;
}

void MainWindow::open_reserves_list(){
    if(GLOBAL_MODE != SEARCHER_WHITE_MODE)
        return;
    Searcher *s = static_cast<Searcher*>(search_tabs->currentWidget());
    s->open_reserves_list();
}
...
void MainWindow::switch_indicator(){
    QImage rect_image(15,15,QImage::Format_RGB16);
    QPainter painter;
    painter.begin(&rect_image);
    painter.setBrush(QBrush(show_hidden ? Qt::green : Qt::red, Qt::SolidPattern));
    painter.drawRect(rect_image.rect());
    painter.end();
    indicator->setPixmap(QPixmap::fromImage(rect_image));
    search_tabs->switch_hidden();
}
...
void MainWindow::create_search_widget(){
    ...
    indicator = new QLabel(search_tabs);
    switch_indicator();
    search_tabs->setCornerWidget(indicator, Qt::TopRightCorner);
    ...
    for(int i = 0; i < settings->value(QString("%1/tabs_default").arg(USERNAME), 5).toInt(); i++){
        s = search_tabs->add_tab(&reserve_struct, &white_columns, &grey_columns);
        if(s->success == false)
            break;
...
void MainWindow::add_tab(){
    search_tabs->add_tab(&reserve_struct, &white_columns, &grey_columns);
}
