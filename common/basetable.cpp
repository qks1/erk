...
void SmartTableView::wheelEvent(QWheelEvent *e){
    int delta = e->delta();
    int row = 0;
...
void SmartTableView::keyPressEvent(QKeyEvent *event){
...
void BaseTable::set_filled(){
    if(!filled)
        filled = true;
}

void BaseTable::setItemDelegateForColumn(int column, QAbstractItemDelegate *d){
    table->setItemDelegateForColumn(column, d);
}
...
inline void BaseTable::base_connects(){
    ...
    connect(table, SIGNAL(pressed(QModelIndex)), SIGNAL(clicked(QModelIndex)));
    connect(table, SIGNAL(doubleClicked(QModelIndex)), SIGNAL(double_clicked(QModelIndex)));
}
...
void BaseTable::set_model(QAbstractItemModel *model){
...
void BaseTable::hide_column(int i){
    if(table->model()->columnCount() >= i)
        table->hideColumn(i);
}

void BaseTable::show_column(int i){
    if(table->model()->columnCount() >= i)
        table->showColumn(i);
}
...
void BaseTable::hide_show_columns(){
    ...
    if(columns->appears.count() < table->model()->columnCount()){
        for(int i = columns->appears.count(); i < table->model()->columnCount(); i++)
            columns->appears << true;
    }

    for(int i = 0; i < table->model()->columnCount(); i++)
        table->horizontalHeader()->setSectionHidden(i, !(columns->appears.at(i)));
}
...
QVariant BaseTable::table_data(int row, int column){
    if(this->table->model()->rowCount() == 0 || this->table->model()->columnCount() == 0)
        return QVariant();
...
QModelIndex BaseTable::current_index(){
...
void BaseTable::mouse_released_slot(){
    ...
    if(resized_index != old_index || resized_width != old_width){
        ...
        qDebug() << columns->sizes.count();
        qDebug() << resized_index;
