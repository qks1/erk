...
QPushButton* create_panel_button(QWidget *parent, QLayout *lt, QString tooltip, QString image_file, const char *method){
    ...
    button->setFixedSize(top_panel_size, top_panel_size);
...
QString create_pattern(QString src, QString add_symbols){
    // удаляем из строки всё, кроме букв, цифр и, возможно, дополнительных символов (переданных в add_symbols)
    QString pattern = QString("[^a-zA-Zа-яА-Я0-9%1]").arg(add_symbols);
    QString rez = src.remove(QRegExp(pattern));
    ///qDebug() << rez;
    return rez;
...
RightAlignmentDelegate::RightAlignmentDelegate(QObject *parent)
    : QItemDelegate(parent)
{
}

void RightAlignmentDelegate::paint(QPainter *painter,
                 ...
                 const QModelIndex & index) const
{
    ...
    QItemDelegate::paint(painter, my_option, index);
}
