...
double YearsDiscountsItem::dscnt_round(double base){
    // если цена не больше рубля - округляем до 5 копеек
    // если не больше 10 рублей - округляем до 10 копееек
    // иначе до рубля
    if(base <= 1.00)
        return floor((base+0.025)*20) / 20.0;
    if(base <= 10.00)
        return floor((base+0.05)*10) / 10.0;
    if(base > 10.00)
...
void YearsDiscountsItem::count_retail(){
    int sign = (discount_mode ? -1 : 1);
    double bp = base_retail * (1 + sign*(percents_spinbox->value()/100.00));
    retail_label->setText(QString::number(dscnt_round(bp), 'f', 2));
}

void YearsDiscountsItem::count_whole(){
    int sign = (discount_mode ? -1 : 1);
    double bp = base_whole * (1 + sign*(percents_spinbox->value()/100.00));
    whole_label->setText(QString::number(dscnt_round(bp), 'f', 2));
}

void YearsDiscountsItem::count_prices(){
    if(change_retail) count_retail();
    if(change_whole) count_whole();
...
void YearsDiscountsItem::set_retail_enabled(bool enabled){
    ...
    if(!manual_checkbox->isChecked())
        enabled ? count_retail() : retail_label->setText(QString::number(base_retail, 'f', 2));
}
...
void YearsDiscountsItem::set_whole_enabled(bool enabled){
    ...
    if(!manual_checkbox->isChecked())
        enabled ? count_whole() : whole_label->setText(QString::number(base_whole, 'f', 2));
}
