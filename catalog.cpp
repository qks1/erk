...
void Catalog::move_button(){
    // поместить кнопку сброса фильтра в правый угол заголовка

    // ширина и высота кнопки должны быть равны высоте заголовка
    int size = header->height();

    // отстоять от левого края кнопка должна на (ширина_каталога-ширина_скроллбара)
    // как определить ширину скроллбара - не знаю, посему подгоняем на глазок
    int wdth = this->groups->width()-(size*1.7);
